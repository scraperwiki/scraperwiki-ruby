...
=end
    ...
    class SqliteSaveInfo
        ...
        def buildinitialtable(data)
            ...
            scoldef = ", ".join(  coldef.map { |col| format("`%s` %s", col[0], col[1]) } )
...
=begin
        def findclosestindex(unique_keys)
            idxlist = @sqliteexecute("PRAGMA main.index_list(`%s`)" % @swdatatblname)  # [seq,name,unique]
            uniqueindexes = [ ]
            if 'error' in idxlist
                return None, None
            ...
            for idxel in idxlist["data"]
                if idxel[2]
                    idxname = idxel[1]
                    idxinfo = @sqliteexecute("PRAGMA main.index_info(`%s`)" % idxname) # [seqno,cid,name]
        ...
        def makenewindex(idxname, unique_keys)
            ...
            lres = @sqliteexecute("create unique index `%s` on `%s` (%s)" % (newidxname, @swdatatblname, ",".join(["`%s`"%k  for k in unique_keys])))
            if "error" in lres  
                return lres
            if idxname
                lres = @sqliteexecute("drop index main.`%s`" % idxname)
                if "error" in lres  
